...
class BinaryTree(object):
    ...
    def CalculateSum(self,start,tmp):
        ...
        if start is None:
            return
        if start.right!=None and (start.right.left is None and start.right.right is None):
            tmp.append(start.right.value)
        self.CalculateSum(start.left,tmp)
        self.CalculateSum(start.right,tmp)

    def RightLeavesBinaryTree(self):
    ...
    def RightLeavesBinaryTreeWithoutRecurrsion(self):
        ...
        while len(node_lst)>0:
            ...
            while len(node_lst)>0:
                ...
                n=node_lst[-1]
                node_lst.pop()
                if n.right!=None and (n.right.left is None and n.right.right is None):
                    num_lst.append(n.right.value)
                if n.left!=None:
                    tmp.append(n.left)
                if n.right!=None:
                    tmp.append(n.right)
...
def main():
    ...
    print(tree.RightLeavesBinaryTree())
    print(tree.RightLeavesBinaryTreeWithoutRecurrsion())
